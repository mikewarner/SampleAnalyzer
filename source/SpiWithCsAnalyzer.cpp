...
SpiWithCsAnalyzer::SpiWithCsAnalyzer()
:	Analyzer2(),
	mSettings( new SpiWithCsAnalyzerSettings() ),
...
void SpiWithCsAnalyzer::SetupResults()
{
	mResults.reset( new SpiWithCsAnalyzerResults( this, mSettings.get() ) );
...
void SpiWithCsAnalyzer::WorkerThread()
{
	Setup();
...
void SpiWithCsAnalyzer::AdvanceToActiveEnableEdgeWithCorrectClockPolarity()
{
...
void SpiWithCsAnalyzer::AdvanceToActiveEnableEdge()
{
	if( mEnable != NULL )
	{
...
bool SpiWithCsAnalyzer::IsInitialClockPolarityCorrect()
{
...
bool SpiWithCsAnalyzer::WouldAdvancingTheClockToggleEnable()
{
	if( mEnable == NULL )
		return false;
...
void SpiWithCsAnalyzer::GetWord()
{
...
bool SpiWithCsAnalyzer::NeedsRerun()
{
	return false;
}

U32 SpiWithCsAnalyzer::GenerateSimulationData( U64 minimum_sample_index, U32 device_sample_rate, SimulationChannelDescriptor** simulation_channels )
{
...
U32 SpiWithCsAnalyzer::GetMinimumSampleRateHz()
{
	return 10000; //we don't have any idea, depends on the SPI rate, etc.; return the lowest rate.
}

const char* SpiWithCsAnalyzer::GetAnalyzerName() const
{
	return "SPI With CS Result";
}
