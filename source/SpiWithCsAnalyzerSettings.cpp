...
SpiWithCsAnalyzerSettings::SpiWithCsAnalyzerSettings()
...
bool SpiWithCsAnalyzerSettings::SetSettingsFromInterfaces()
{
...
void SpiWithCsAnalyzerSettings::LoadSettings( const char* settings )
{
	...
	if( strcmp( name_string, "SaleaeSpiWithCsAnalyzer" ) != 0 )
		AnalyzerHelpers::Assert( "SaleaeSpiWithCsAnalyzer: Provided with a settings string that doesn't belong to us;" );
...
const char* SpiWithCsAnalyzerSettings::SaveSettings()
{
	SimpleArchive text_archive;

	text_archive << "SaleaeSpiWithCsAnalyzer";
	text_archive <<  mMosiChannel;
	text_archive <<  mMisoChannel;
	text_archive <<  mClockChannel;
	text_archive <<  mEnableChannel;
...
void SpiWithCsAnalyzerSettings::UpdateInterfacesFromSettings()
{
	mMosiChannelInterface->SetChannel( mMosiChannel );
	mMisoChannelInterface->SetChannel( mMisoChannel );
	mClockChannelInterface->SetChannel( mClockChannel );
